...
const memoRectCallback = (instance: Virtualizer<any, any>, cb: (rect: Rect) => void) => {
	...
	return (rect: Rect) => {
		if (
			instance.options.horizontal ? rect.width !== prev.width : rect.height !== prev.height
		) {
			cb(rect);
		}
...
export const measureElement = (element: unknown, instance: Virtualizer<any, any>) => {
...
export class Virtualizer<TScrollElement = unknown, TItemElement = unknown> {
	...
	private measureElementCache: Record<number, (measurableItem: TItemElement | null) => void> =
		{};
	...
	getVirtualItems = memo(
		...
		(indexes, measurements, measureElement) => {
			const makeMeasureElement = (index: number) => (measurableItem: TItemElement | null) => {
