...
import {
	DEFAULT_MIDDLEWARE,
	RelativeHrefSanitizerPlugin,
	type PurifyMiddleware,
} from "./purify-middleware";
...
export class PurifyHTML {
	static create(middleware: PurifyMiddleware[] = DEFAULT_MIDDLEWARE, baseURI: string = "") {
		return new PurifyHTML([...middleware, new RelativeHrefSanitizerPlugin(baseURI)]);
	}
	private middleware: PurifyMiddleware[];
	private htmlParser?: Parser;

	constructor(middleware: PurifyMiddleware[]) {
		...
		nodes.forEach((node) => {
			if (node.type === "tag") {
				...
				for (let i = 0; i < this.middleware.length; i++) {
					const plugin = this.middleware[i];
