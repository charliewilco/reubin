type EvictionHandler<KeyType, ValueType> = (
  key: KeyType,
  value: ValueType
) => void;
...
export class QuickLRU<KeyType, ValueType>
  implements Iterable<[KeyType, ValueType | undefined]> {
...
export class SimpleLRU<T = unknown>
  extends QuickLRU<string, T>
  implements ICacheStore<T> {
  constructor() {
    super({ maxSize: 1000 });
  }
